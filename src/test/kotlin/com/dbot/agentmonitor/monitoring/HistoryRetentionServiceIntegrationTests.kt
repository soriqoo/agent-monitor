...
@SpringBootTest
@ActiveProfiles("test")
class HistoryRetentionServiceIntegrationTests {

    @Autowired
    lateinit var jdbcTemplate: JdbcTemplate

    @Autowired
    lateinit var historyRetentionService: HistoryRetentionService
    ...
    @Test
    fun pruneDeletesOldChecksAlertsAndResolvedIncidentsButKeepsOpenIncidents() {
        val now = OffsetDateTime.parse("2026-05-19T12:00:00+09:00")
        val old = now.minusDays(31)
        val recent = now.minusDays(3)

        insertServiceCheck("old-check", old)
        insertServiceCheck("recent-check", recent)
        insertAlert("old-alert", old)
        insertAlert("recent-alert", recent)
        insertResolvedIncident("old-resolved", old, old.plusMinutes(5))
        insertResolvedIncident("recent-resolved", recent, recent.plusMinutes(5))
        insertOpenIncident("old-open", old)

        val result = historyRetentionService.prune(now)

        assertThat(result.deletedServiceChecks).isEqualTo(1)
        assertThat(result.deletedAlertEvents).isEqualTo(1)
        assertThat(result.deletedResolvedIncidents).isEqualTo(1)
